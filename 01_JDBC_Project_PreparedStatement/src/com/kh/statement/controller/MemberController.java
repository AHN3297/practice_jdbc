 ...
 * 1. 메소드로 전갈왼 데이터를 가공처리한 뒤 DADO로 전달압니다.
 * 2. DAO로부터 반환받은 결과에 따라서 VIEW로 응답결과를 반환 
 */
public class MemberController {
	
	/**
	 *  VIEW로부터 전달받은 문자열 값 다섯 개를 DAO로전달하는메소드
	 *  
	 * @param userId : 사용자에게 입력받은 아이디 값(8자....~~자 까지)
	 * @param userPwd : 사용자에게 입력받은 비밀번호 값
	 * @param userName : 사요자에게 입력받은 이름 값
	 * @param email : 사용자에게 입력받은 이메일값
	 *  
	 * @return : 반환 어떻게 할건지
	 * @throws : IllgalArgumentException : 잘못된 인자가 전달되면 발생할 수있음
	 */
	public int save(String userId, String userPwd, String userName, String email) {
		// 1. 데이터 가공 => 넘어온 인자값이 두 개 이상일 시 
		// 				  어딘가에 담을 것(VO, Map, DTO)
		// 전달된 인자값들을 Member 객체의 필드에 담기
		// 1) 매개변수 생성자 호출하여 객체 생성과 동시에 필드값을 대입 (요청처리)
	...
	public int update(String userId, String userPwd, String newPassword) {
		...
		// 아이디랑 비밀번호 맞는지 확인 바꿀 비밀번호로 비밀번호컬럼값 바꿔줘
		// DAO호출
