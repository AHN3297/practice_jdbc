...
/*
 * 비즈니스 로직 실행(의사결정코드) -> 데이터 가공, 중복체크, 연산 처리, 암호화
 * 트랜잭션 관리
 * 여러 DAO를 조합
 * 예외처리 및 변환
 * 보안 및 권한 검사
...
public class MemberService {
	 ...
	 * 			 DAO에서 DB작업이 끝나면 Sercice단에서 결과에 따른 트랜잭션 처리도 진행
	 * 
	 * 			 => Service를 추가함으로 DAI는 순수하게 SQL문을 처리하는 부분만 남겨둘것
	 * 
	 */
	...
	public int save(Member member) {
		//JDBCTemplate.registDriver();//main으로 날려
	...
	public Member findById(String userId) {
		// 1) Connection만들기
		//Connection conn = getConnection();
		// 진짜 1) DAO호출(Service생성자에서 받아온 Connection + Controller가 준
	...
	public List<Member> findByKeyword(String keyword){
		// 할일
		// 01) Connection만들기는 이미 기본생성자에서 함
	...
	public int update(PasswordDTO pd) {
		...
		// 비밀번호 수정
		//UPDATE MEMBER SET USERPWD = 머시기 AND USERPWD = 머시기
		if(pd.getNewPassword().length() >20) {
			//throw new RuntimeException("너무 긴 비밀번호에요");
			return 0;
		}
		Member member = new MemberDao().findById(conn, pd.getUserId());
		if(member ==null) {
			//throw new RuntimeException("존재하지않는아이디입니다.");
			return 0;
		}
	...
	public int delete(Member member) {
		...
		// 4) 트잭션 끝나면 Connection할 일 없으니까 반납
		close(conn);
		// 5) 결과반환
		return result;
	}
